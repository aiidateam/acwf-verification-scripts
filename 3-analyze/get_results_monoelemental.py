...
STRUCTURES_GROUP_LABEL = f'commonwf-oxides/{SET_NAME}' # To change back appending /{PLUGIN_NAME} if needed
...
if __name__ == "__main__":
    ...
    for node in progress_bar:
        ...
        if element != 'O':
             # The only one valid for monoelemental, apart from oxygen
            assert configuration == 'X', f"element '{element}' does not have configuration 'X' as expected"
    ...
    fname = f"outputs/monoelemental-results-warnings-{PLUGIN_NAME}.txt"
    ...
    fname = f"outputs/monoelemental-results-{PLUGIN_NAME}.json"
    with open(fname, 'w') as fhandle:
