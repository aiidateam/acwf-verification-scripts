...
element_list = [
    'Ac', 'Ag', 'Al', 'Am', 'Ar', 'As', 'At', 'Au', 'B', 'Ba', 'Be', 'Bi', 'Bk', 'Br',
    'C', 'Ca', 'Cd', 'Ce', 'Cf', 'Cl', 'Cm', 'Co', 'Cr', 'Cs', 'Cu', 'Dy', 'Er', 'Es', 'Eu',
    'F', 'Fe', 'Fm', 'Fr', 'Ga', 'Gd', 'Ge', 'H', 'He', 'Hf', 'Hg', 'Ho', 'I', 'In',
    'Ir', 'K', 'Kr', 'La', 'Li', 'Lr', 'Lu', 'Md', 'Mg', 'Mn', 'Mo', 'N', 'Na', 'Nb', 'Nd',
    'Ne', 'Ni', 'No', 'Np', 'O', 'Os', 'P', 'Pa', 'Pb', 'Pd', 'Pm', 'Po', 'Pr', 'Pt', 'Pu',
    ...
    if not os.path.exists(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), os.pardir,
             "outputs", "TS-plots-%s-%s" % (SET_NAME, folder_string))):
        print("ERROR! No folder ../outputs/TS-plots-%s-%s found." % (SET_NAME, folder_string))
    ...
    with open('tex-template/results.tex', 'w') as fhandle:
        ...
        for element in element_list:
            for configuration in configurations:
                ...
                fhandle.write("\\IfFileExists{../../outputs/TS-plots-%s-%s/%s-%s.pdf}"  % (SET_NAME, folder_string, element, conf_for_fname))
                fhandle.write("{\\includegraphics[width=0.15\\linewidth]{../../outputs/TS-plots-%s-%s/%s-%s}}" % (SET_NAME, folder_string, element, conf_for_fname))
