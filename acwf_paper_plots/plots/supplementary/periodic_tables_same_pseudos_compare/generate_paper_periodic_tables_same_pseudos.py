...
def plot_periodic_table(SET_NAME, QUANTITY, code_ref, code_comp, comment=''):
    ...
    if USE_AE_AVERAGE_AS_REFERENCE:
        ...
        reference_data_files = labels_data['methods-supplementary'][code_ref]
        reference_short_label = labels_data['methods-supplementary'][code_ref]['short_label']
    try:
    ...
    short_labels[code_comp] = labels_data['methods-supplementary'][code_comp]['short_label']
    with open(os.path.join(DATA_FOLDER, labels_data['methods-supplementary'][code_comp][SET_NAME])) as fhandle:
        code_results[code_comp] = json.load(fhandle)
        if not code_results[code_comp]['script_version'] in EXPECTED_SCRIPT_VERSION:
            raise ValueError(
                f"This script only works with data generated at version {EXPECTED_SCRIPT_VERSION}. "
                f"Please re-run ./get_results.py to update the data format for {code_comp}! Skipping it"
                )
    ...
    for plugin, plugin_data in code_results.items():
        ...
        for conf in list_confs:
            ...
            data_to_export.update(dict(zip(
                ...
                collect[conf]["values"])))
        with open(f"{QUANTITY}-{SET_NAME}-{short_labels[plugin].replace(' ', '_')}-vs-{reference_short_label.replace(' ', '_')}-{comment}.json", 'w') as fhandle:
        ...
        reference_label = 'all-electron average' if USE_AE_AVERAGE_AS_REFERENCE else code_ref
        ...
        if SHOW_IN_BROWSER:
            ...
            try:
                export_png(p, filename=f"periodic-table-{SET_NAME}-{short_labels[plugin].replace(' ', '_')}-vs-{reference_short_label.replace(' ', '_')}-{comment}-{QUANTITY}.png")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Pass as first parameter the code to be used as reference (e.g. 'SIRIUS/CP2K@PW|SSSP-prec-v1.2').")
        sys.exit(1)
        
    try:
        code_comp = sys.argv[2]
    except IndexError:
        print("Pass as second parameter the code to be used as comparison (e.g. 'Quantum ESPRESSO@PW|SSSP-prec-v1.2').")
        sys.exit(1)
        
    try:
        comment = sys.argv[3]
    except IndexError:
        print("Pass as third parameter a comment to be added to the output file name. Or it will be empty.")
        comment = ''
    ...
    for SET_NAME in ['unaries', 'oxides']:
        for QUANTITY in [
            ...
            plot_periodic_table(SET_NAME, QUANTITY, code_ref, code_comp, comment)