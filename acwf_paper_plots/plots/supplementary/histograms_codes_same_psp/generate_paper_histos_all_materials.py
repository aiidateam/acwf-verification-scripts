...
def generate_histo(sets, name_file, code_ref, code_comp):
    """
    """
    ...
    for SET_NAME in sets:
        ...
        with open(os.path.join(DATA_FOLDER, labels_data['methods-supplementary'][code_ref][SET_NAME])) as fhandle:
    ...
    #fig.suptitle(f"code_ref vs code_comp")
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Pass as second parameter the code to be used as reference (e.g. 'SIRIUS/CP2K@PW|SSSP-prec-v1.2').")
        sys.exit(1)
        
    try:
        code_comp = sys.argv[3]
    except IndexError:
        print("Pass as third parameter the code to be used as comparison (e.g. 'Quantum ESPRESSO@PW|SSSP-prec-v1.2').")
        sys.exit(1)
        
    try:
        comment = sys.argv[4]
    except IndexError:
        print("Pass as fourth parameter a comment to be added to the output file name. Or it will be empty.")
        comment = ''

    if mode == 'all':
        generate_histo([SET_NAME_1, SET_NAME_2], f'all-mat-{comment}', code_ref, code_comp)
    elif mode == 'separate':
        generate_histo([SET_NAME_1], f'unaries-mat-{comment}', code_ref, code_comp)
        generate_histo([SET_NAME_2], f'oxides-mat-{comment}', code_ref, code_comp)
    else:
        print("Pass as first parameter 'separate' (separate analysis for unaries and oxides) or 'all'.")
        sys.exit(1)
