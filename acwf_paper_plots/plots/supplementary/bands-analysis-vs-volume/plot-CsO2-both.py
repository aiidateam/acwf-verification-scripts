...
for filename in os.listdir(FOLDER):
    ...
    if ext != '.json':
        continue
    # Consider only first and last
    if not (basename.startswith('1-') or basename.startswith('7-')):
        continue
    ...
    for path in paths:
        ...
        for band, band_type in zip(path['values'], all_data['band_type_idx']):
            ...
            if basename.startswith('1-'):
                further_plot_options['color'] = 'blue'
            else:
                further_plot_options['color'] = 'green'

            # Only first line of all segments

            further_plot_options['label'] = None
            if first and all_data['title']:
                first = False
                further_plot_options['label'] = all_data['title']
...
## PLOT A BAR TO INDICATE THE SMEARING
smearing = 0.0045 * 13.605698066 # ~61 meV
p.axhspan(-smearing, +smearing, alpha=0.2, color='red', label="$\\pm \\sigma$")
...
pl.savefig(f'{system}-both.pdf')
print(f"file {system}-both.pdf written.")
