...
print_comment = False
with open("Er-diamond-bands_data.json", encoding='utf8') as f:
    all_data_str = f.read()
...
fermi_energy = all_data['fermi_energy']
...
for path in paths:
    ...
    for band, band_type in zip(path['values'], all_data['band_type_idx']):
        ...
        p.plot(x, np.array(band) - fermi_energy, label=label,
...
try:
    ...
    pass
pl.savefig('Er-diamond-bands.pdf')